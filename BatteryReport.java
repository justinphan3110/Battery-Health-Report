...
public class BatteryReport extends Application{
	...
	public static void execute() throws IOException, ScriptException {
		...
		reader.close();
		String path = "path";
		int firstIndex = result.indexOf(path) + path.length() + 1; // get the word after the path
		int lastIndext = result.length() - 1; // -1 to trip the . behind
...
//		File batteryReport = new File("C:\\Users\\LongP\\Downloads\\battery-report.html");
	    ...
	    File batteryReport = new File(url);
		Document batteryHTML = Jsoup.parse(batteryReport, null);
//	            System.out.println(batteryHTML);
		Elements elements = batteryHTML.select("td");

		String trimReport = "";
		BufferedWriter writer = new BufferedWriter(new FileWriter("BatteryReport.txt"));
...
//		for (String a : overView.keySet()) {
//			System.out.println(a + ": " + overView.get(a));
//		}
...
//		for (String[] listTemp : currentCapac) {
...
//			System.out.println();
		...
		writeCSV(CSVNAME);
		// create a Renjin engine and analyzed with R
		RenjinScriptEngineFactory factory = new RenjinScriptEngineFactory();
		ScriptEngine engine = factory.getScriptEngine();
//		engine.eval("print(getwd())");
    ...
    if (result.get() == scanButton) {
	  ...
	  private static void resultBox(){
	        ...
	        resultBox.setContentText(computerName + ": " + overView.get(computerName) + "\n" +
	        						systemModel + ": " + overView.get(systemModel)  + "\n" +
	        		 				bios + ": " + overView.get(bios) + "\n" + 
	        						reportTime + ": " + overView.get(reportTime));
	        ButtonType result = new ButtonType("short result", ButtonBar.ButtonData.FINISH);
	        ButtonType data = new ButtonType("Details Data", ButtonBar.ButtonData.OK_DONE);
	        resultBox.getDialogPane().getButtonTypes().setAll(result, data);
	  ...
	  private static void shortDetailBox() {
		  ...
		  resultBox.setContentText(batteryName + ": " + batteryInfo.get(batteryName)+ "\n" + 
				  				   manufacture + ": " +batteryInfo.get(manufacture)+ "\n" +
				  				   serialNumber + ": " +batteryInfo.get(serialNumber) + "\n" +
				  				   chemistry + ": " +batteryInfo.get(chemistry) + "\n" +
				  				   designCapacity + ": " + batteryInfo.get(designCapacity) + "\n" +
				  				   fullChargeCapacity + ": " + batteryInfo.get(fullChargeCapacity) + "\n" +
...
class Chart extends JFrame {
	public Chart(String title) {
		...
		JFreeChart chart = ChartFactory.createScatterPlot("Battery Capacity vs Weeks", "weeks", "Battery Capacity(mWh)",
				dataSet);
